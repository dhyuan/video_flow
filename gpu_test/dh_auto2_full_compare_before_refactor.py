...
def compare(d_test_features, d_sources_features_on_gpu, files_index, file_features_index_dict, value,
    ...
    print('AA--%d' % len(file_features_index_dict))
    ...
    init_results = np.zeros((total_source_features_len, 3), dtype=np.float)
    ...
    # h_results = np.array([0, 0, 0] * total_source_features_len, dtype=np.float32)
    ...
    print('在 %d 个文件中比对 %s 用时: %s\n' % (len(files_index), test_file, (end_time - compare_start_time)))
...
def filter_out_impossible_data(results, test_len, value):
    result_len = len(results)
    print('filter_out_impossible_data len of input %d' % result_len)

    time_begin_create_np_array = datetime.now()
    possible_results = filter_as_numpy_array(results, value)
    time_end_create_np_array = datetime.now()
    print('create_np_array %s' % (time_end_create_np_array - time_begin_create_np_array))

    possible_results.max(axis=1)
...
def __filter_array(results, candidate_threshold):
    result_len = len(results)
    ...
    for i in range(result_len):
        ...
        if frame_probability > candidate_threshold:
            possible_result.append([frame_probability, frame_index, file_index])
    possible_result_len = len(possible_result)
    print('---deduced down %.2f  %d %d'
          % (((result_len - possible_result_len) % result_len), result_len, possible_result_len))
    return possible_result


def filter_as_numpy_array(results, candidate_threshold):
    array_data = __filter_array(results, candidate_threshold)
...
def trans_all_source_features_as_whole(file_feature_dict):
    ...
    print('特征值 %d 导入GPU用时：%s' % (mem_size, (time_end - time_begin)))
...
def read_sources_features_from_dir(features_dir, fun_features_str2tuple):
    ...
    print(features_dir)
    for file in files:
        if os.path.isdir('%s/%s' % (features_dir, file)) or not file.endswith('.txt') or file.startswith('.') or os.path.getsize(
