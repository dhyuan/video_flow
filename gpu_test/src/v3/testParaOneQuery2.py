...
def compare(test_file, d_sources_features_on_gpu, file_feature_dict, top_numb):
    ...
    for file_name, d_source_features in d_sources_features_on_gpu.items():
        ...
        gpu_results = np.array([0.0] * source_len, dtype=np.float)
        d_gpu_result = cuda.to_device(gpu_results)
        cuda.synchronize()
        ...
        d_gpu_result.copy_to_host(gpu_results)
        cuda.synchronize()
        ...
        ts_top_n_result = find_most_probability(gpu_results, top_numb)
