...
def compare(d_test_features, d_sources_features, file_feature_dict, top_numb):
    ...
    final_top_n_result = []
    for file_name, d_source_features in d_sources_features.items():
    ...
    print('\n在 %d 个文件中比对 %s 用时: %s' % (sources_numb, test_file, (compare_end_time - compare_start_time)))
...
def find_most_probability_from_block_result(results, frame_results, top_numb):
    ...
    for i in range(len(results)):
        ...
        print('frame=%d   p=%f' % (frame_index, a))
        if a > top_results[0]:
...
def compare_frame_by_kernel(source_len, test_len,
    ...
    s_block_result[cuda.threadIdx.x] = result_of_from_idx
    s_block_frame_idx_result[cuda.threadIdx.x] = idx
    ...
    if cuda.threadIdx.x == 0:
        ...
        for i in range(1, BLOCK_SIZE):
            #print(s_block_frame_idx_result[i])
            #print(s_block_result[i])
...
if __name__ == '__main__':
    ...
    for test_file in test_files:
        ...
        compare(d_test_features_on_gpu, d_sources_features_on_gpu, file_feature_dict, 10)
