    ...
    if len(sys.argv) > 1:
        ...
        for test_file in test_files:
            ...
            for source_file in source_files:
                probability, frame_index = search_file('/home/ai/test/%s' % test_file, '/home/ai/source/%s' % source_file)
                if probability > final_result:
                    final_result = probability
                    final_frame = frame_index
                    final_source_file = source_file
            print('FResult: %s %s %f %d' % (test_file, final_source_file, final_result, final_frame))
