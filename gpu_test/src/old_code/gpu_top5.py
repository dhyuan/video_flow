...
import math
import os
from numba import float32, float64, uint32
...
def calculate_source_position_by_gpu(list_source, list_test):
    source_features = np.array(list(map(lambda d: trans_source_data_to_tuple(d), list_source)), dtype=np.float64)
    test_features = np.array(list(map(lambda d: trans_test_data_to_tuple(d), list_test)), dtype=np.float64)
    ...
    d_gpu_result = cuda.device_array(source_len, dtype=np.float64)
...
@cuda.jit(restype=float64, argtypes=[uint32, uint32, float64[:], float64[:]],  device=True)
...
@cuda.jit(device=True, argtypes=[float64, float64, float64])
...
if __name__ == '__main__':
    ...
    search_file('./data/东京湾景_02.txt', './data/冰上恋人_01.txt')
    # source_files = ['./data/冰上恋人_01.txt', './data/东京湾景_02.txt']
    # source_files = get_files_from_dir('/Users/dahui/GitHub/video_flow/gpu_test/data')
    # test_files = get_files_from_dir('testdir')
    # for source_file in source_files:
    #     for test_file in test_files:
    #         print('%s  ==>  %s' % (test_file, source_file))
