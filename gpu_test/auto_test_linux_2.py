...
def read_sources_features_from_dir(features_dir, fun_features_str2tuple):
    ...
    file_feature_dict = {}
    for file in files:
        if file.startswith('.') or os.path.getsize('%s/%s' % (features_dir, file)) == 0:
            print('忽略空文件: %s' % file)
            continue
...
def update_source_result(start,end):
    get_source = MySource()
    database_source_result = get_source.getAllOkIds(start,end)
...
if __name__ == '__main__':
    ...
    if IS_WIN32:
        ...
        resule_path = '/storage/auto_test/tmp/result/'
    ...
    update_database_source_feature(sources_dir,0,1000)
    d_sources_features_on_gpu, file_feature_dict = load_features_to_gpu(sources_dir)

    create_graph()
    get_test = MyTest(args[0],int(args[1]))
    ...
    while True:
        ...
        compare(test_file, d_sources_features_on_gpu, file_feature_dict, value)
